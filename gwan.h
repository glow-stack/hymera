...
typedef struct
{
   ...
   u32      h_entity            :12, // 0-4096 (room for HTTP headers
            h_host              :12, //         or pipelined requests)
            h_useragent         :12,
            h_referer           :12,
            ...
            mime                : 8, // 0-255 mime type index in table
            charset             : 1, // 0-1   needs charset in HTT headers
            gzipable            : 1, // 0-1   gzip'able
            gzip_asked          : 1, // 0-1   client supports gzip
            nodelay             : 1, // 0-1   TCP_NODELAY was enabled
            cork                : 1, // 0-1   TCP_CORK was enabled
...
// Protocol handlers
// ----------------------------------------------------------------------------
// The list below can be extended with new protocols (ie: "FTP") by (a)
// implementing the corresponding protocol handler in a gwan/.../handlers
// folder. Note that already implemented protocols can be replaced by your
// own implementation by just registering a protocol using the same name.

enum PROTOCOL_def
{
   P_GWAN = 0, // default (HTTP)
   P_GW,       // Global-WAN
   P_DNS,      // free slot
   P_NTP,      // stub (non-standard) implementation
   P_HTTP,     // free slot
   P_HTTPS,    // free slot
   P_WAIT,     // free slot
   P_CGI,      // free slot
...
// find if a given protocol handler has been defined AND used by a listener;
// examples:
//   const int used = proto_used("DNS", 0); // check by name
//   const int used = proto_used(NULL, 10); // check by index
//
// Note: a protocol is in 'use' if (a) the corresponding protocol handler has
//       been implemented either by G-WAN or in the gwan/.../handlers folder 
//       and (b) this protocol handler has been assigned to at least one G-WAN
//       listener.
//
// registration test (-1:!found, else index)
int proto_used(const char *name, int idx); 

// convert a protocol name into its id value; this can be used to switch the
// current protocol in use in G-WAN servlets.
// example:
//   const int id = proto_2idx("DNS");
//   if(id >= 0) // else this protocol has not been defined yet
//      C->protocol = id; // connections launched by the servlet use DNS
int proto_2idx(const char *name); // id of name     (-1:!found)

// convert a protocol id into its name; this can be used to check which
// protocol is currently in use in G-WAN servlets.
// example:
//   const char *name = proto_2name(C->protocol);
char *proto_2name(const u32 i);

#define INET_ADDRSTRLEN  16 // IPv4
#define INET6_ADDRSTRLEN 46 // IPv6
#define UNIX_ADDRSTRLEN 108 // Unix sockets

typedef struct // IP address (string and numeric representation)
{
   u8  str[INET_ADDRSTRLEN]; // "123.123.123.123"
   u32 num;
} gcc_packed ip_t;

typedef struct // customizable connection context (see ctx[1])
{
   ip_t   ip;           // "a.b.c.d" + IP address as u32 (see above)
   s64    tm_birth,     // connection birth (ms)
          tm_io,        // last I/O time (ms)
          tm_out;       // when to close the connection (ms)
   int    fd;           // client socket
   u32    protocol : 7, // 0-128 entry in PROTOCOL_def enum {}
          state    : 5, // 0-32  connection state (user-defined)

          // I/O connection events as received from the network
          accepted : 1, // 0-1   a new connection has been accepted
          connected: 1, // 0-1   a connect() call succeeded
          can_recv : 1, // 0-1   data is available in socket buffer
          can_send : 1, // 0-1   free room is available in socket buffer
          disabled : 1, // 0-1   temporarily on-wait for a foreign even
          error    : 1, // 0-1   error (brutal cut)
          closing  : 1, // 0-1   closed-by-client or by-error
          
          reserved :25; // reserved, do not use!
          
   char   ctx[1];       // user-defined connection context
} gcc_packed conn_t;
...
// HTTP status codes supported by the G-WAN http_status() API call
// (these defines can also be used for G-WAN servlet return codes)

// This list has non-standard entries due to that fact that Apache, IIS, 
// Nginx, Google and Twitter have defined their own codes, sometimes in
// a redundant manner with the RFCs; all these codes are listed here to
// let you use whatever you are used at, as this might be useful in your
// Web applications and log files.

// Informal
#define HTTP_100_CONTINUE                      100
#define HTTP_101_SWITCHING_PROTOCOLS           101 // HTTP/1.1 => HTTPS (RFC 2817)
...
#define HTTP_203_NON_AUTHORITATIVE             203
...
#define HTTP_226_IM USED                       226
#define HTTP_230_AUTHENTICATION_SUCCESSFUL     230
...
#define HTTP_306_SWITCH_PROXY                  306 // "no longer used"
#define HTTP_307_TEMPORARY_REDIRECT            307
#define HTTP_308_PERMANENT_REDIRECT            308 // "experimental"

// Client Errors
#define HTTP_400_BAD_REQUEST                   400
#define HTTP_401_UNAUTHORIZED                  401
#define HTTP_402_PAYMENT_REQUIRED              402 // Google CAPTCHAs
...
#define HTTP_418_I_AM_A_TEAPOT                 418 // April joke
#define HTTP_420_ENHANCE_YOUR_CALM             420 // TWitter: reduce rate
#define HTTP_422_UNPROCESSABLE_ENTITY          422
#define HTTP_423_LOCKED                        423
#define HTTP_424_FAILED_DEPENDENCY             424
#define HTTP_425_UNORDERED_COLLECTION          425
#define HTTP_426_UPGRADE_REQUIRED              426 // HTTP/1.1 => HTTPS (RFC 2817)
#define HTTP_429_TOO_MANY_REQUESTS             429
#define HTTP_431_REQUEST_HEADER_FIELDS_2_LARGE 431
#define HTTP_444_NO_RESPONSE                   444 // Nginx: client timeout
#define HTTP_449_RETRY_WITH_APPROPRIATE_ACTION 449 // MSFT extension
#define HTTP_450_BLOCKEDBY_WINDOWS_PARENTALCTL 450 // MSFT extension
#define HTTP_451_UNAVAILABLE_FOR_LEGAL_REASONS 451 // Internet draft
#define HTTP_494_REQUEST_HEADER_TOO_LARGE      494 // Nginx: see 431
#define HTTP_495_CLIENT_CERT_ERROR             495 // Nginx: SSL error
#define HTTP_496_CLIENT_LACKS_CERT             496 // Nginx: SSL error
#define HTTP_497_HTTP_TO_HTTPS_REDIRECT        497 // Nginx: see 426
#define HTTP_499_CLIENT_CLOSED_REQUEST         499 // Nginx: bad client close
...
#define HTTP_505_VERSION_NOT_SUPPORTED         505
#define HTTP_506_VARIANT_ALSO_VARIES           506
#define HTTP_507_INSUFFICIENT_STORAGE          507
#define HTTP_508_LOOP_DETECTED                 508
#define HTTP_509_BANDWIDTH_LIMIT_EXCEEDED      509 // Apache extension
#define HTTP_510_NOT_EXTENDED                  510
#define HTTP_511_NETWORK_AUTH_REQUIRED         510
#define HTTP_598_NETWORK_READ_TIMEOUT_ERROR    510 // MSFT extension
#define HTTP_599_NETWORK_CONNECT_TIMEOUT       510 // MSFT extension
...
int s_snprintf (void *str, size_t len, const char *fmt, ...);
int s_vsnprintf(void *str, size_t len, const char *fmt, va_list a);
...
enum SMTP_server_caps
{
   ...
   CAP_AUTH_PLAIN       = (1 << 7),
   CAP_AUTH_LOGIN       = (1 << 8),
   CAP_AUTH_CRAM_MD5    = (1 << 9),
   CAP_AUTH_DIGEST_MD5  = (1 << 10),
   CAP_AUTH_SCRAM_SHA1  = (1 << 11),
   CAP_AUTH_GSSAPI      = (1 << 12),
   CAP_AUTH_EXTERNAL    = (1 << 13), // just 'username' sent via TLS
   CAP_AUTH_NTLM        = (1 << 14), // Microsoft Windows
   CAP_STATUS_COD       = (1 << 15),
   CAP_8BITMIME         = (1 << 16),
};
...
typedef struct
{
   ...
   char *file; // file contents loaded in memory
   int   size; // file contents size
} attach_t;

// char     **dst  = {"tom@ab.ch","pam@ab.ch", 0};
// attach_t **att  = {0}; // no attachments here
// char error[256] = {0}; // 256-byte (or larger) buffer.
//
// See the email.c and contact.c servlet examples for more details.

int sendemail(char *mailserver,     // "mail.ab.com:25"
              char *src_mail_addr,  // "tim@ab.com"
              char **dst_mail_addr, // dst (an array of email addresses)
              char *subject,        // "test"
              char *text,           // "test"
              int   text_len,       // sizeof("test") - 1
              attach_t **attach,    // att (an array of attachment(s))
              char *auth_user,      // "user"
              char *auth_pass,      // "password"
              int   auth_meth,      // an SMTP_server_caps value
              char *error);         // for error message (>= 256 chars)
...
// keylen - values 128, 192 or 256 (AES-128 is faster and safer than AES-256)
